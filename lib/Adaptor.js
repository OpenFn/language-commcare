 ...
 * @returns {Operation}
 */
function addRow(db, table, rowData) {
  ...
  return function (state) {
    ...
    var authToken = _state$configuration.authToken;
    var apiVersion = _state$configuration.apiVersion;


    var url = 'https://reportsapi.commcare.com/api/'.concat(account, '/', db, '/', table);
    ...
    return (0, _Client.post)({ url: url, body: body, authToken: authToken, apiVersion: apiVersion, action: action }).then(function (result) {
